...
static int prologue(struct solutionCtrlBlock_t *_blk, int argc, char *argv[])
{
    ...
    _ctx->_eng = aoc_2d_eng_create(&_lim, '.', 10, bycoordinatesYfirst, true);
...
static int epilogue(struct solutionCtrlBlock_t *_blk)
{
    ...
    dll_head_h allobjs = aoc_2d_eng_list_objects(eng, "^", "S");
    dll_head_h allspliters = aoc_2d_eng_list_objects(eng, "^", "S");
    size_t progress = 0;
    LL_FOREACH_P_REV(objn, allobjs)
    {
        aoc_2d_obj_h objh = (aoc_2d_obj_h)((aoc_2d_obj_ref_h)objn)->data;
        aoc_2d_obj_h firsthit = beamexplore(ctx, objh);
        if (!ret)
            return ret;
        memoreport(ctx, objh, progress) + 2;
        progress++;
    }

    dll_free_all(allspliters, free);
    free(allspliters);
    DYN2DFREE_DEALLOC(ctx->_memosplits, memospawn_h, ctx->limits._max._x + 1, ctx->limits._max._y + 1, freememo);

    dll_free_all(allobjs, free);
    free(allobjs);
