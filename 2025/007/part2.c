...
static int prologue(struct solutionCtrlBlock_t *_blk, int argc, char *argv[])
{
    ...
    _ctx->_eng = aoc_2d_eng_create(&_lim, '.', 10, NULL, true);
...
static int epilogue(struct solutionCtrlBlock_t *_blk)
{
    ...
    aoc_2d_obj_h highestsplitter = NULL;
    size_t progress = 0;
    LL_FOREACH_P_REV(objn, ctx->_allobjs)
    {
        aoc_2d_obj_h objh = (aoc_2d_obj_h)((aoc_2d_obj_ref_h)objn)->data;
        highestsplitter = beamexplore(ctx, objh); 
        memoreport(ctx, objh, progress);
        progress++;
    } 
