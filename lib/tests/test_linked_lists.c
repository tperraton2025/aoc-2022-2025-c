...
#define TS_CAST(_p) ((stringnode_t *)_p)

#define ASSERT_LL_INTEGRITY(_ll, ...)                                       \
    {                                                                       \
        stringnode_h _exp[] = {__VA_ARGS__};                                \
...
static stringnode_h _string_ll_test(const char *const _str);
...
void ll_test_suite()
{
    ...
    stringnode_h _str1 = _string_ll_test("foo 1");
    stringnode_h _str2 = _string_ll_test("foo 2");
    stringnode_h _str3 = _string_ll_test("foo 3");
    stringnode_h _str4 = _string_ll_test("foo 4");

    stringnode_h _strings[] = {_str1,
                               _str2,
                               _str3,
                               _str4};
...
static stringnode_h _string_ll_test(const char *const _str)
{
    stringnode_h _nstr = malloc(sizeof(stringnode_t));
...
static void print(void *arga, void *argb)
{
    stringnode_h _nstr = (stringnode_h)arga;
    printf("%s", _nstr->_str);
}

static void free_test_string(void *_node)
{
    stringnode_h _nstr = (stringnode_h)_node;
    FREE(_nstr->_str);
    _nstr->_str = NULL;
    FREE(_node);
    _node = NULL;
